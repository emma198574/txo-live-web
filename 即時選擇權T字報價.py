...
網頁分成「週三結算」「週五結算」「下週三結算」「月選／隔月結算」四個分頁，
依到期日由近到遠取合約（週三＝W 系列週選或月選，週五＝F 系列週選，
最後一頁＝最近一個還沒被前三頁佔掉的月選），量比、價平、▲▼ 增減都各算各的。
...
def is_monthly(gkey, grp):
    """是不是月選（TXO）。

    月選的商品代碼固定是 TXO，週選是 TX1/TX4/TXX/TXY 這一類；
    MIS 的中文名對月選不帶日期（'臺指選086'），contract_label 因此回 'N月選'，
    拿來當第二道確認 —— 哪天代碼規則變了不至於整個分頁憑空消失。
    """
    return gkey[0] == "TXO" or (grp.get("name") or "").endswith("月選")


def pick_monthly(groups, exclude=()):
    """最近一個「還沒出現在其他分頁」的月選；沒有就回 None。

    為什麼不是固定取第 2 近的月選（那才是字面上的「隔月」）：
    月選結算週（例 09/16）當天，近月月選就是「週三結算」那頁本身，
    再取第 2 近才會是 10 月；但結算完之後 10 月選變成近月，
    這時該顯示的就是 10 月而不是 11 月 —— 11 月選現在整天只有 29 口，
    畫出來全是雜訊。改以「其他分頁已經佔掉的到期日」排除，
    兩種情況都會落在同一個答案上：最近的、還看得到量的那個月選。
    """
    today = datetime.now(TW_TZ).strftime("%Y%m%d")
    by_exp = {}
    for g, v in groups.items():
        exp = v["exp"]
        if not is_monthly(g, v) or v["vol"] <= 0 or exp < today or exp in exclude:
            continue
        cur = by_exp.get(exp)
        if cur is None or v["vol"] > cur[1]["vol"]:
            by_exp[exp] = (g, v)
    return by_exp[min(by_exp)] if by_exp else None


def month_tab_name(exp):
    """月選分頁的標題：跟今天同一個月就叫「月選結算」，否則是「隔月結算」。

    寫死成「隔月結算」會在 10 月的時候指著 10 月選說隔月，明明就是當月。
    """
    return "月選結算" if int(exp[4:6]) == datetime.now(TW_TZ).month else "隔月結算"
...
def build_page(radius=1500):
    ...
    # 月選分頁：排除上面三頁已經佔掉的到期日，免得結算週那天跟第一頁畫出同一張表
    mth = pick_monthly(groups, exclude={v["exp"] for _, _, _, v in picks})
    if mth:
        picks.append((MONTH_TAB, month_tab_name(mth[1]["exp"]), mth[0], mth[1]))
...
<div class="note">
  ...
  <b>分頁</b>：四個分頁是不同結算日的合約，照結算先後排（本週三＝W 系列或月選、
  本週五＝F 系列、下週三＝下一個週三到期的合約、最後一頁＝月選），
  各自獨立計算量比、價平與 ▲▼ 增減；切換後的選擇會記住，自動重整不會跳回去。
  <b>下週三</b>是還沒進入結算週的部位，量通常只有本週的零頭，牆的位置常是先卡好的
  初始陣地，看的是下一段的區間預期，不要拿來當今天的當沖依據。
  <b>月選（隔月）</b>那一頁挑的是「最近一個還沒出現在前三頁的月選」——
  月選結算週當天它就是下個月，結算完之後換成剛變近月的那個月，所以標題會在
  「月選結算」與「隔月結算」之間換，看小字的到期日最準。這一頁的重點不是盤中撐壓
  （剛掛的月選一天可能只有幾百口），而是<b>昨日 OI 那道牆</b>：法人的月線級部位
  先卡在哪裡。量能不足時盤中撐壓與莊家意圖都會自動擋掉不顯示，只留 OI 牆。<br>
